...
As observed in Table 1, the analysis revealed significant findings regarding the interplay between cyclone characteristics and landfall probability. First, the Pearson correlation coefficient shows a strong negative correlation of approximately -0.92 between wind speed and pressure. This suggests that as wind speed increases, pressure tends to decrease, indicating a significant inverse relationship. Second, the T-test comparing wind speeds between cyclones with and without landfall shows an extremely small p-value of approximately 1.28〖×10〗^(-44). This indicates a substantial difference in wind speed between cyclones with and without landfall, providing strong evidence for the impact of wind speed on landfall occurrence.

Lastly, the results of the Ordinary Least Squares (OLS) regression analysis revealed a statistically significant model, with a low R-squared value of 0.013. This indicates that only a small proportion (about 1.3%) of the variance in landfall distance is explained by the predictors in the model. While the R-squared value indicates that only a small proportion of the variance in landfall distance is explained by the predictors in the model, it’s important to consider the fact that hurricanes are complex systems influenced by numerous factors, so even a relatively low R-squared may still provide valuable insights. Moreover, the coefficients (2.1936 for maximum sustained wind and 7.2618 for minimum pressure) indicate the expected change in the nearest landfall distance for a one-unit increase in each predictor variable, holding other variables constant. So, an increase in maximum sustained wind speed by one unit would lead to a change in the nearest landfall distance by 2.1936 units, and similarly for minimum pressure. 

## Modeling the probability of landfall

The methodology employed to address research question, which seeks to explore models for predicting the probability of landfall events such as logistic regression, decision tree, and random forest classifiers, entails several key steps. Firstly, after data preprocessing, it is split into training and testing sets. Subsequently, three classification models are trained: logistic regression, decision tree, and random forest. For decision tree, hyperparameter tuning is conducted using grid search. Furthermore, to address class imbalance, engineered features (‘Nearest_landfall_distance’ and ‘Intensification_rate’) are added, and the models are retrained. Model evaluation involves assessing accuracy, precision, recall, and F1 score, along with generating ROC curves. Additionally, learning curves are plotted to analyze the models’ performance with varying training sizes. 